...
$(function () {
  ...
  $('#quotationBtn').on('click', function () {
    ...
    if (errors.length > 0) {
      ...
      $('#output').show();
      createText($('input[name=kbn]:checked').val(), $('#money').val(), $('#charge').val(), $('#currency').val(), $('#count').val(), $('#fees').val());
    }
...
function createText(chargeLabel, money, charge, currency, count, fees) {
  ...
  let result = Math.ceil(((moneyNum + chargeNum) * (currencyFloat + 15)) / 100) + 3000 * countNum + feesNum;
  ...
  $('#output').html(
    `총 입금해주실 금액은 ${result.toLocaleString()}원입니다.\n\n</br></br>` +
      ...
      `(의뢰금액 ${moneyNum.toLocaleString()}엔${
        chargeLabel == 1 ? ` + 편의점수수료 ${chargeNum.toLocaleString()}엔` : chargeLabel === 2 ? ` + 송금수수료 ${chargeNum.toLocaleString()}엔` : ''
      }) * 환율 + 대행수수료 ${(3000 * countNum).toLocaleString()}원${feesNum === 0 ? '' : ` + 추가수수료 ${feesNum.toLocaleString()}원`} \n\n</br></br>` +
      `💡환율\n</br>` +
