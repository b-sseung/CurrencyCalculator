...
const HomeContainer = () => {
    ...
    function createText(chargeLabel, money, charge, currency) {
        ...
        $("#output").html(
            ...
            `💡견적\n</br>` +
            `(의뢰금액 ${moneyNum.toLocaleString()}엔${chargeLabel === '0'
    ...
    return (
        <AreaParent className='flexBox _col'>
            ...
            <Body>
                <div className='flexBox _col parent'>
                    ...
                    <div className='gridBox _col3' style={{ gridGap: '12px' }}>
                        {radios.map((text, index) => {
                            return <RadioItem key={index} text={text} index={index} name='kbn' onChange={onChangeKbn}></RadioItem>
                        })}
                    </div>
                    <Error id='kbn'></Error>
                </div>
                <div className='flexBox _col parent'>
                    <label>의뢰 금액</label>
                    <Input type="number" inputMode="numeric" id="money" placeholder="예: 15100" />
                    <Error id='money'></Error>
                </div>
                <div className='flexBox _col parent'>
                    <label>추가 수수료</label>
                    <Input type="number" inputMode="numeric" id="charge" placeholder="예: 220" />
                    <Error id='charge'></Error>
                </div>
                <div className='flexBox _col parent'>
                    <label>현재 환율</label>
                    <Input type="number" inputMode="numeric" id="currency" placeholder="예: 900.01" />
                    <Error id='currency'></Error>
                </div>
            </Body>
            <Area className='flexBox _col' style={{marginTop: '48px'}}>
                <Button id="calBtn" onClick={onClick} >견적 계산</Button>
                <Button id="copyBtn" onClick={onClick} >문장 복사</Button>
            </Area>
            <Text>
